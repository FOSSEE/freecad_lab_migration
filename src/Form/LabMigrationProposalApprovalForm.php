...
// var_dump($proposal_id);die;
  ...
  Url::fromUserInput('/lab-migration/download/syllabus-copy-file/' . $proposal_id)
)->toString(),
        // var_dump( $proposal_id );die;
      ];
    ...
    $form['cancel'] = [
      ...
      '#markup' => Link::fromTextAndUrl(
  $this->t('Cancel'),
  Url::fromUserInput('/lab-migration/manage-proposal')
)->toString(),
...
$config = \Drupal::config('Lab_migration.settings');
      ...
      $bcc = trim($user_email . ', ' . $additional_emails, ', ');
      ...
      if ($form_state->getValue(['approval']) == 2) {
        ...
        // RedirectResponse('lab-migration/manage-proposal');
        $url = Url::fromRoute('lab_migration.manage_proposal_pending')->toString();
      \Drupal::service('request_stack')->getCurrentRequest()->query->set('destination', $url);
        return;
      }
    }
